...
export const getAdminDetail = async (adminId) => {
  try {
    console.log(`---member-inquiry/adminId API 호출, adminId: ${adminId}`);

    const res = await jwtAxios.get(
      `${prefix}/member-inquiry/adminId/${adminId}`
    );

    console.log(`---member-inquiry/adminId/${adminId} API 응답:`, res);

    return res.data; // 필요한 데이터만 반환 (res.data로 수정)
  } catch (error) {
    console.error(`---member-inquiry/adminId/${adminId} API 호출 오류:`, error);
    throw error; // 에러를 상위 로직에서 처리하도록 던짐
  }
};
